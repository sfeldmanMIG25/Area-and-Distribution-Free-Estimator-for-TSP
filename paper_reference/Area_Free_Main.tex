...
\begin{abstract}
Existing TSP tour-length estimators depend on geometric enclosure metrics---such as convex hull area---that become intractable beyond two dimensions ($O(n^{\lfloor d/2 \rfloor})$) and produce degenerate estimates on non-uniform distributions. We propose the Geometrically Assisted Regression Tree (GART 2.0), a LightGBM-based framework that decomposes estimation into two phases: deterministic computation of the Minimum Spanning Tree (MST) and statistical prediction of the MST-to-TSP scaling factor $\alpha = L_{\text{TSP}} / L_{\text{MST}}$. GART 2.0 uses 29 dimension-agnostic features combining lightweight geometric descriptors (bounding hypervolume, centroid statistics) with MST topology---edge weight moments, degree statistics, and diameter---replacing convex hull computation with coordinate-range statistics computable in $O(n \cdot d)$. For 2D instances, we exploit the Delaunay triangulation to construct the MST in $O(n \log n)$ time rather than $O(n^2)$.

We evaluate GART 2.0 on three benchmarks: 2{,}580 synthetic 2D instances across five distribution classes (including near-linear geometries that lie outside the model's training distribution), 16{,}907 multi-dimensional test instances spanning $d \in \{2, \ldots, 100\}$, and 110 TSPLIB95 instances \citep{reinelt1991tsplib}. For the 32 non-Euclidean TSPLIB instances (CEIL\_2D, ATT, GEO, EXPLICIT-metric), we apply classical multidimensional scaling (MDS) \citep{torgerson1952mds,borg2005mds} to embed the distance matrix into Euclidean coordinates, enabling estimation where purely geometric baselines cannot operate. Instances that violate the triangle inequality (e.g., \texttt{brg180}) are excluded; our pipeline assumes a symmetric metric.

Beyond accuracy (3.23\% MAPE on 2D, 0.81\% on multi-dimensional data), GART 2.0's primary contribution is \textit{precision}: 4.94\% SDPE on 2D and 1.17\% on multi-dimensional data---$2.2\times$ and $5.9\times$ tighter than the MST Ratio baseline (Levene's test, $p < 10^{-127}$ and $p \approx 0$). On TSPLIB95, GART 2.0 achieves 3.44\% MAPE / 3.52\% SDPE on the 78 EUC\_2D instances and 4--7\% MAPE on each non-Euclidean distance type, generalizing to $n = 85{,}900$ (86$\times$ the training cap). Prediction time is under 500~ms for $n = 1{,}000$. A precise estimator with a small systematic offset can be recalibrated; an imprecise one cannot.
\end{abstract}


% keywords can be removed
\keywords{Traveling Salesman Problem\and Tour cost estimation \and Distribution-free \and Multidimensional networks}
...
Our work contributes to the literature on distribution-free estimators, first introduced by \citet{cavdar2015distribution}. We advance this methodology by replacing features that scale poorly with dimensionality---convex hulls ($O(n^{\lfloor d/2 \rfloor})$) and Principal Component Analysis (PCA) \citep{jolliffe2002}---with MST-derived topological statistics and lightweight coordinate-range descriptors computable in $O(n \cdot d)$.
...
We propose the Geometrically Assisted Regression Tree (GART 2.0), a framework that estimates TSP tour cost using the relationship between the MST and the optimal tour. GART 2.0 integrates gradient-boosted regression trees with a dimension-agnostic feature set derived from the MST topology to overcome the dimensional limitations of classical geometric estimators. Unlike traditional approaches that map coordinate-dependent metrics directly to tour cost, GART 2.0 decouples the estimation into two phases: (1) the deterministic computation of the MST, and (2) the statistical prediction of the scaling coefficient $\alpha = L_{\text{TSP}} / L_{\text{MST}}$. The final estimator is $\hat{L}_{\text{TSP}} = \hat{\alpha} \cdot L_{\text{MST}}$, where $\hat{\alpha}$ is predicted by a LightGBM regressor based on 29 topological features of the instance. The following section introduces these features.
...
\textbf{Normalized Diameter} & Diameter / Total Length. Scale-invariant shape proxy ($\approx 1.0$ for lines). & $O(1)$ & \textbf{Proposed}; via double-BFS \\
...
The inference complexity of the LightGBM model is determined by the total number of split comparisons. Our final model has $K=4{,}431$ trees with at most $L=258$ leaves per tree. Each tree requires at most $\lceil \log_2 L \rceil = 9$ comparisons to traverse from root to leaf, yielding $K \times 9 \approx 40{,}000$ total comparisons---a constant independent of $n$. The total asymptotic complexity of GART 2.0 is therefore $O(n^2 \cdot d)$.
...
Throughout this evaluation, we report two complementary error metrics. The \textit{Mean Absolute Percentage Error} (MAPE) measures overall accuracy: $\text{MAPE} = \frac{1}{N}\sum_{i=1}^{N} \left|\frac{\hat{L}_i - L_i}{L_i}\right| \times 100$. The \textit{Standard Deviation of Percentage Error} (SDPE) measures \textit{precision}---the consistency of predictions: $\text{SDPE} = \operatorname{std}\left(\frac{\hat{L}_i - L_i}{L_i} \times 100\right)$. Low MAPE indicates accurate predictions on average; low SDPE indicates that the errors are tightly concentrated, even if systematically offset. We emphasize SDPE because a precise estimator with a small systematic offset can be corrected by a single recalibration constant, while an imprecise estimator cannot. Statistical significance of SDPE differences is assessed using Levene's test for equality of variances.
...
\end{equation}
where $V$ is the volume of the convex hull (or bounding box in high dimensions) and $\beta_d$ is a dimension-dependent constant. For $d=2$, the BHH constant has been estimated as $\beta_2 \approx 0.7080$ by \citet{percus1996} through Monte Carlo simulation. We use $\beta_2 = 0.7124$, which falls within the range of published estimates \citep{percus1996, carlsson2024upper}.
...
GART 2.0 transfers from synthetic to real-world at 3.44\% MAPE / 3.50\% SDPE overall --- tighter than every classical baseline including MST Ratio (5.22\% / 4.51\%). The largest instance ($n = 85{,}900$, 86$\times$ the training cap) is predicted in under 0.5~s. GART 2.0 wins both MAPE and SDPE in the two smaller-$n$ buckets ($[51, 150]$ and $[151, 400]$) and wins SDPE in all four buckets. In the $n > 1500$ bucket MST Ratio beats GART on MAPE (2.93\% vs 4.47\%) because true $\alpha$ in large TSPLIB EUC\_2D instances concentrates near the asymptotic value of 1.07--1.15 \citep{percus1996}, so a fixed constant is hard to beat on accuracy there. BHH, Cavdar, and Chien all degrade past $n = 400$ as real-world instances violate the uniform-density assumption their derivations require.
...
GART 2.0 achieves 3.23\% MAPE / 4.94\% SDPE on the 2D diverse benchmark, 0.81\% MAPE / 1.17\% SDPE on multi-dimensional data ($d$ up to 100), and 3.44\% MAPE / 3.52\% SDPE on the 78 EUC\_2D TSPLIB95 instances (with $n$ up to 85{,}900, 86$\times$ the training cap). Precision advantages are statistically overwhelming: $2.2\times$ tighter than MST Ratio in 2D (Levene $p < 10^{-127}$) and $5.9\times$ tighter in ND ($p \approx 0$). On the 32 non-Euclidean TSPLIB95 instances (CEIL\_2D, ATT, GEO, EXPLICIT-metric), GART 2.0 is, to our knowledge, the first tour-length estimator evaluated across every metric distance type in the library via a single MDS-hybrid pipeline. The Delaunay-accelerated feature pipeline yields sub-500~ms prediction at $n = 1{,}000$.

\subsection{Limitations and Future Work}

GART 2.0 offers no speedup over exact solvers for $n < 50$. Grid lattices and near-linear manifolds produce 8--11\% error due to MST topological degeneracy. On TSPLIB EXPLICIT-metric instances, MDS embedding introduces information loss (5.94\% MAPE); non-metric distance matrices (bridge graphs, asymmetric costs) are incompatible with MST-based estimation by construction. The model was trained on $n \le 1{,}000$ and $d \le 50$; extrapolation to $d = 100$ holds sub-1\% MAPE.

Incorporating geometries that are currently rare in training (grid lattices, near-linear manifolds) should reduce error on those boundary cases. The natural next step is to use GART 2.0 as a cost oracle inside a larger optimization loop: either as the subroutine estimating sub-tour cost in a Vehicle Routing Problem \citep{frontiers2021}, or as the bounding function in a branch-and-bound solver. Both tests would measure whether an estimator with this level of precision translates into faster, more reliable end-to-end decisions.
