...
The full feature corpus comprises $90{,}418$ solved instances with node counts $n \in [5, 1000]$ and dimensions $d \in \{2, 3, 4, 5, 6, 7, 8, 9, 10, 15, 20, 25, 30, 35, 40, 45, 50\}$ (17 training-visible dimension values), plus a held-out extrapolation bucket at $d = 100$. Instances are partitioned 70/20/10 into training ($59{,}247$), validation ($16{,}714$), and test ($14{,}457$) via a stratified split on $(d, n, \text{grid size})$; all $d = 100$ instances are locked into the test partition so the model never sees that dimension during training or validation. Generator details and the stratification rule are documented in Appendix~\ref{app:training}; extrapolation to $d = 100$ is evaluated in Section~\ref{subsec:results_nd}.
...
\begin{table}[!ht]
\centering
\caption{GART 2.0 hyperparameters (Optuna TPE, 100 trials, RMSE objective) and trained-model statistics. Values match the committed \texttt{lgbm\_model\_v3/best\_params\_v3.json} and the saved \texttt{lgbm\_alpha\_model\_v3.joblib} booster.}
\label{tab:hyperparams}
\begin{tabular}{lr}
\toprule
Parameter & Value \\
\midrule
\multicolumn{2}{l}{\textit{Tuned hyperparameters}} \\
learning rate        & $0.0129$ \\
num leaves           & $142$ \\
L1 regularisation    & $8.49\times10^{-8}$ \\
...
\begin{table}[!ht]
\centering
\caption{Top-10 features of GART 2.0 by mean $|$SHAP$|$, computed on the held-out test split ($N=5{,}000$ sampled rows, seed 42) with the trained booster. Share (\%) is the share of total mean absolute SHAP mass.}
\label{tab:shap_top}
\begin{tabular}{rlrr}
\toprule
Rank & Feature & Mean $|$SHAP$|$ & Share (\%) \\
\midrule
1  & \texttt{mst\_dominance\_ratio}     & 0.0319 & 23.2 \\
2  & \texttt{n\_customers}              & 0.0161 & 11.7 \\
3  & \texttt{dimension}                 & 0.0132 &  9.6 \\
4  & \texttt{mst\_degree\_mean}         & 0.0079 &  5.7 \\
5  & \texttt{centroid\_dist\_std}       & 0.0068 &  4.9 \\
6  & \texttt{bounding\_hypervolume}     & 0.0061 &  4.4 \\
7  & \texttt{mst\_diameter}             & 0.0057 &  4.1 \\
8  & \texttt{mst\_total\_length}        & 0.0050 &  3.7 \\
9  & \texttt{large\_edge\_count}        & 0.0049 &  3.6 \\
10 & \texttt{mst\_diameter\_normalized} & 0.0049 &  3.5 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!ht]
\centering
\caption{TSPLIB95 EUC\_2D benchmark by instance size (78 instances, four balanced buckets). SDPE is primary (first numeric column) and includes a 95\% bootstrap CI (1000 resamples, seed 42). GART 2.0 rows are bolded. Kwon (1995) is included for completeness; its calibration is known to break for $n$ well beyond typical VRP sizes --- its $n > 1500$ bucket returns a clipped estimator (all predictions clamped to $0$), reflected in $100\%$ error, which we report as-observed rather than hide. \textit{Optimal solver} row shows mean \textsc{Concorde} wall time in milliseconds.}
...
\multirow{9}{*}{$[51,\,150]$}
  ...
  & Kwon              & 14.27 \scriptsize[6.21, 18.53]         & 14.83          & 14.01 & 0.965  & ---   & 0.63  & 23 \\
  & Daganzo           & 12.80 \scriptsize[6.16, 16.72]         & 27.94          & 29.61 & 0.894  & ---   & 0.51  & 23 \\
  & \textit{Optimal solver} & --- & --- & --- & --- & --- & 854 & --- \\
\midrule
\multirow{9}{*}{$[151,\,400]$}
  & \textbf{GART 2.0} & \textbf{3.23} \scriptsize[2.00, 4.11]  & \textbf{2.89}  & 2.50  & 0.997  & 0.841 & 18.4  & 16 \\
  ...
  & Kwon              & 23.30 \scriptsize[14.58, 28.50]        & 24.84          & 27.84 & 0.879  & ---   & 0.68  & 16 \\
  & Daganzo           & 18.63 \scriptsize[9.26, 23.67]         & 21.33          & 23.29 & 0.908  & ---   & 0.55  & 16 \\
  & \textit{Optimal solver} & --- & --- & --- & --- & --- & 15{,}525 & --- \\
\midrule
\multirow{9}{*}{$[401,\,1500]$}
  & \textbf{GART 2.0} & \textbf{3.92} \scriptsize[2.55, 4.84]  & \textbf{3.60}  & 2.25  & 0.999  & 0.666 & 34.5  & 20 \\
  & MST Ratio         & 3.51 \scriptsize[2.58, 4.14]           & 4.85           & 5.65  & 0.993  & ---   & 101.7 & 20 \\
  ...
  & Kwon              & 28.77 \scriptsize[14.15, 39.39]        & 81.60          & 100.00 & $-1.03$ & --- & 0.87  & 20 \\
  & Daganzo           & 39.34 \scriptsize[16.26, 51.87]        & 27.65          & 20.11 & 0.955  & ---   & 0.72  & 20 \\
  & \textit{Optimal solver} & --- & --- & --- & --- & --- & $> 1$~h (not measured) & --- \\
\midrule
\multirow{9}{*}{$n > 1500$}
  & \textbf{GART 2.0} & \textbf{3.28} \scriptsize[2.05, 3.96]  & \textbf{4.47}  & 3.50  & 1.000  & 0.486 & 87.5  & 19 \\
  & MST Ratio         & 3.52 \scriptsize[2.29, 4.40]           & 2.93           & 2.21  & 0.998  & ---   & 539.9 & 19 \\
  ...
  & Hilbert           & 14.47 \scriptsize[9.42, 19.08]         & 48.76          & 49.11 & 0.597  & ---   & 36.0  & 19 \\
  & Kwon$^{\dagger}$  & 0.00 \scriptsize[0.00, 0.00]           & 100.00         & 100.00 & $-0.10$ & --- & 2.60  & 19 \\
  & Daganzo           & 40.38 \scriptsize[12.95, 59.20]        & 23.48          & 11.64 & 0.994  & ---   & 2.25  & 19 \\
  & \textit{Optimal solver} & --- & --- & --- & --- & --- & $> 1$~h (not measured) & --- \\
\midrule
\multirow{9}{*}{\textbf{Total}}
  & \textbf{GART 2.0} & \textbf{3.52} \scriptsize[2.95, 4.04]  & \textbf{3.44}  & 2.37  & 1.000  & 0.727 & 36.7  & 78 \\
  & MST Ratio         & 4.54 \scriptsize[3.54, 5.36]           & 5.22           & 4.96  & 0.998  & ---   & 158.8 & 78 \\
  ...
  & Hilbert           & 11.84 \scriptsize[9.83, 13.62]         & 45.20          & 44.60 & 0.623  & ---   & 10.9  & 78 \\
  & Kwon$^{\dagger}$  & 44.87 \scriptsize[40.73, 47.93]        & 54.75          & 32.51 & $-0.03$ & --- & 1.18  & 78 \\
  & Daganzo           & 32.60 \scriptsize[21.00, 42.89]        & 25.42          & 23.68 & 0.994  & ---   & 0.99  & 78 \\
  & \textit{Optimal solver} & --- & --- & --- & --- & --- & $> 1$~h (not measured) & --- \\
\bottomrule
\multicolumn{9}{l}{\scriptsize $^{\dagger}$ Kwon (1995) was calibrated for VRP customer counts $\approx 50$--$300$; its linear $-0.0011n$ term drives the estimate negative at large $n$, clipped to $0$.} \\
\end{tabular}%
}
\end{table}
...
For each non-Euclidean TSPLIB instance we obtain a Euclidean coordinate representation by applying classical multidimensional scaling \citep{torgerson1952mds,borg2005mds,cox2000mds} to the distance matrix $D$, retaining the smallest number of dimensions $k$ that preserves $\ge 99.9\%$ of the eigenvalue variance (capped at $k = 100$). Geometric features are computed on the MDS embedding while MST features are computed from the \emph{original} $D$ so no metric information is lost, and the selected $k$ is passed to the model as the \texttt{dimension} feature. Full derivation and the per-instance $k$ distribution are given in Appendix~\ref{app:mds}.
...
The $90{,}418$ solved instances are partitioned into training ($59{,}247$, $65.5\%$), validation ($16{,}714$, $18.5\%$), and test ($14{,}457$, $16.0\%$) using a stratified sample on $(d, n, \text{grid size})$. The stratification rule ranks instances within each stratum by a seeded uniform and assigns the first $10\%$ to test, the next $20\%$ to validation, and the remainder to training; $d = 100$ instances bypass this rule and are locked to the test partition (see \texttt{feature\_creator\_v3.create\_stratified\_split}).
