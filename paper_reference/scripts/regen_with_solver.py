"""Regenerate all five LaTeX tables with a 'Solver (s)' column added after Time (ms).
...
def fmt_solver_s(s):
    """Format solver time in seconds; compact across 6 orders of magnitude."""
    ...
    if s < 1e6:
        return fr"{s/1000:.0f}k"
    return fr"{s/1e6:.1f}M"
...
def compute_bucket_row(sub, model_code, solver_s):
    ...
    return dict(N=len(sub), MAPE=mape, SDPE=sdpe, MED=med, R2=r2, R2A=r_alpha, T=t_ms, SOLVER=solver_s)


def emit_row(stats, model_code, bold_all=False):
    ...
    if bold_all and model_code == "LGBM_V3":
        ...
        if r_a != "---":
            r_a = fr"\textbf{{{r_a}}}"
        t = fr"\textbf{{{t}}}"
    return f"    & {dn} & {n} & {mape} & {sdpe} & {med} & {r2} & {r_a} & {t} & {s} \\\\"


def emit_bucket_table(df, buckets, models, caption, label, bucket_var, solver_per_bucket, solver_total):
    out = [r"\begin{table}[!ht]",
           ...
           r"\footnotesize",
           r"\begin{tabular}{@{}ll rrrrrrrr@{}}",
           r"\toprule",
           fr"{bucket_var} & Model & $N$ & MAPE (\%) & SDPE (\%) & Median (\%) & $R^2$ & $r_\alpha$ & Time (ms) & Solver (s) \\",
           r"\midrule"]

    for (lo, hi, lbl) in buckets:
        mask = (df["n"] > lo) & (df["n"] <= hi)
        sub = df[mask]
        solver_s = solver_per_bucket.get(lbl)
        rows = []
        for m in models:
            sm = sub[sub["model"] == m]
            if len(sm) == 0:
                continue
            stats = compute_bucket_row(sm, m, solver_s)
            rows.append((m, stats))
        if not rows:
            continue
        out.append(fr"    \multirow{{{len(rows)}}}{{*}}{{{lbl}}}")
        for m, stats in rows:
            out.append(emit_row(stats, m))
...
# Solver-time per bucket: optimal_solve_time_s deduped by instance
...
print(emit_bucket_table(
    ...
    r"2D diverse benchmark by instance size (2{,}580 instances). Solver time = mean \textsc{Concorde}/\textsc{LKH-3} wall time per bucket (whichever was optimal during training). $r_\alpha$ = Pearson correlation, bounded $[-1, 1]$.",
...
print(emit_bucket_table(
    ...
    r"ND benchmark by instance size (16{,}907 instances). Same data as Table~\ref{tab:nd_by_dim}. Solver time = mean \textsc{Concorde}/\textsc{LKH-3} wall time per bucket.",
...
def emit_nd_by_dim(df, per_inst):
    ...
    out = [r"\begin{table}[!ht]", r"\centering",
           r"\caption{ND benchmark by dimension. Same 16{,}907 instances as Table~\ref{tab:nd_by_size}. $d = 100$ is outside the training range (extrapolation). Solver time = mean \textsc{Concorde}/\textsc{LKH-3} wall time.}",
           r"\label{tab:nd_by_dim}",
           r"\resizebox{\textwidth}{!}{%",
           r"\footnotesize",
           r"\begin{tabular}{@{}ll rrrrrrrr@{}}",
           r"\toprule",
           r"$d$ bucket & Model & $N$ & MAPE (\%) & SDPE (\%) & Median (\%) & $R^2$ & $r_\alpha$ & Time (ms) & Solver (s) \\",
           r"\midrule"]
    ...
    out.append(r"\bottomrule")
    out.append(r"\multicolumn{10}{l}{\footnotesize $^{*}$ $d = 100$ is outside the training range; included to test extrapolation.}")
    out.append(r"\end{tabular}%")
    out.append(r"}")
    out.append(r"\end{table}")
    return "\n".join(out)
...
# compute gap/abs_gap if missing
...
# Per-bucket mean Concorde time on instances that have it
...
for lo, hi, lbl in buckets_tsplib:
    ...
    solver_ts[lbl] = m["concorde_time_s"].mean()
...
print(emit_bucket_table(
    dft, buckets_tsplib,
    ["LGBM_V3", "MST_Ratio", "Cavdar", "BHH", "Chien", "Hilbert"],
    r"TSPLIB95 EUC\_2D benchmark by instance size (78 instances, four balanced buckets). Solver time = mean \textsc{Concorde} wall time; some large instances in our pipeline record no \textsc{Concorde} time (they were solved by \textsc{LKH-3} heuristic to optimality). The $n > 1500$ bucket mean is dominated by a few multi-day runs.",
...
out = [r"\begin{table}[!ht]",
       r"\centering",
       r"\caption{GART 2.0 on non-Euclidean TSPLIB95 (32 instances). $r_\alpha$ = Pearson correlation; undefined when $N < 3$ (ATT). Embedded dimension $k$ chosen by MDS variance retention. \textsc{Concorde} is not applicable to non-Euclidean distance types; solver time is unavailable.}",
...
for dt in ["CEIL_2D", "ATT", "GEO", "EXPLICIT"]:
    ...
    rows_data.append((name, len(sub), ape.mean(), pe.std(ddof=0), ape.median(), r_alpha, t_ms, k_avg, k_min, k_max))
    out.append(f"{name} & {len(sub)} & {ape.mean():.2f} & {pe.std(ddof=0):.2f} & {ape.median():.2f} & {fmt_r(r_alpha)} & {fmt_t(t_ms)} & {k_avg} [{k_min},{k_max}] \\\\")
